...
Model: TF-IDF (word 1-2 grams + char 3-5 grams) → calibrated Linear SVM. Trains in ~30s on CPU, ~5 MB artifact, millisecond inference, a deliberately strong classical baseline before reaching for transformers. Per-class breakdown and a normalized confusion matrix are generated in `reports/` on every training run.
